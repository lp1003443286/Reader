...
import xutils.cache.LruCache;
import xutils.cache.LruDiskCache;
import xutils.common.Callback;
import xutils.common.task.Priority;
import xutils.common.task.PriorityExecutor;
import xutils.common.util.IOUtil;
import xutils.common.util.LogUtil;
import xutils.ex.FileLockedException;
...
/*package*/ final class ImageLoader implements
    ...
    private CommonCallback<Drawable> callback;
    private PrepareCallback<File, Drawable> prepareCallback;
    private CacheCallback<Drawable> cacheCallback;
    private ProgressCallback<Drawable> progressCallback;
    ...
    static Cancelable doLoadDrawable(final String url,
                                     ...
                                     final CommonCallback<Drawable> callback) {
    ...
    static Cancelable doLoadFile(final String url,
                                 final ImageOptions options,
                                 final CacheCallback<File> callback) {
        if (TextUtils.isEmpty(url)) {
            postArgsException(null, options, "url is null", callback);
            return null;
        }
    ...
    private Cancelable doLoad(ImageView view,
        ...
        if (callback instanceof ProgressCallback) {
            this.progressCallback = (ProgressCallback<Drawable>) callback;
        }
        if (callback instanceof PrepareCallback) {
            this.prepareCallback = (PrepareCallback<File, Drawable>) callback;
        }
        if (callback instanceof CacheCallback) {
            this.cacheCallback = (CacheCallback<Drawable>) callback;
        }
    ...
    private static void postArgsException(
            ...
            final String exMsg, final CommonCallback<?> callback) {
